Сервис подписывает JWT-токены вариантом BIP-340 (Schnorr/x-only). Nonce (значение k, дающее R) генерируется детерминированно и повторяется для всех логинов в одной секунду.
Одновременно challenge (значение e) в подписи зависит от строки, которая задаёт сид. Этот сид используется для рандома в методе ROLL, таким образом можно собрать 624 сгенерированных рандомно чисел, и восстановить MT(рандом python).
После восстановления состояния MT, гуглим как восстановить сид, находим например https://github.com/StackeredSAS/python-random-playground/blob/main/recover_BytesV2Seed.py
Используем это, пишем солвер. 
Нам нужны две подписи с одинаковым R(это обеспечивается при логине в одну секунду) и разными m дают линейную систему относительно закрытого ключа x:
s1 = k + e1 * x  (mod n)
s2 = k + e2 * x  (mod n)
=> x = (s1 - s2) * (e1 - e2)^{-1} (mod n)

e находим узнав сид через 
e = int(SHA256("BIP0340/challenge" || rx || Xx || m || SEED)) mod n. e1 находим при m1, e2 при m2 
(m = SHA256(header_b64 + "." + payload_b64))
После вычисления x, подделываем токен и логинимся под админом, забирая заметку с флагом.
Солвер представлен в файле solver.py, а также ниже:
